        ...
        <div id="box">
            <div id="nav">
                ...
                <div id="links">
                    ...
                    <a href="{{ url('profile') }}" class="nav-text">Modifica profilo</a>
                    <a href="{{ url('logout') }}" class="nav-text">Logout</a>
                </div>
            </div>

            <div id="content">
                <h1 class="text-item" id="welc">Benvenuto/a {{ $user->username }}</h1>
                <div class="userbox" id="searchbox">
                </div>
                <div class="userbox" id="resultsbox">
                @if(!$songs == '')
                    <div id="Brani" class="favres"><span class="text-item">I tuoi brani</span></div>
                    @foreach ($songs as $song) 
                            <div id="elembox" name = "track">
                                <div id="buttonbox">
                                    <img name="remove" class="button" id="favbutton" src="http:/example-app/public/img/remove.png">
                                </div>
                                <div id="box_cover">
                                    <a id="elemid" value="{{ $song['id'] }}"></a>
                                    <img id="cover" src="{{ $song['album']['images'][0]['url'] }}">
                                </div>
                                <div id="toptext"><a href = "{{ $song['external_urls']['spotify'] }}" class="track-item" id="spantop">{{ $song['name'] }}</a></div>
                                <div id="bottext"><span class="track-item" id="spanbot">{{ $song['artists'][0]['name'] }}</span></div>
                            </div>
                    @endforeach
                @endif
                @if(!$artists == '')
                    <div id="Artisti" class="favres"><span class="text-item">I tuoi Artisti</span></div>
                    @foreach ($artists as $artist) 
                            <div id="elembox" name = "artist">
                                <div id="buttonbox">
                                    <img name="remove" class="button" id="favbutton" src="http:/example-app/public/img/remove.png">
                                </div>
                                <div id="box_cover">
                                    <a id="elemid" value="{{ $artist['id'] }}"></a>
                                    <img id="cover" src="{{ $artist['images'][0]['url'] }}">
                                </div>
                                <div id="toptext"><a href = "{{$artist['external_urls']['spotify'] }}" class="track-item" id="spantop">{{ $artist['name'] }}</a></div>
                            
                            </div>
                    @endforeach
                @endif
                @if(!$albums == '')
                    <div id="Album" class="favres"><span class="text-item">I tuoi Album</span></div>
                    @foreach ($albums as $album) 
                            <div id="elembox" name = "album">
                                <div id="buttonbox">
                                    <img name="remove" class="button" id="favbutton" src="http:/example-app/public/img/remove.png">
                                </div>
                                <div id="box_cover">
                                    <a id="elemid" value="{{ $album['id'] }}"></a>
                                    <img id="cover" src="{{ $album['images'][0]['url'] }}">
                                </div>
                                <div id="toptext"><a href = "{{$album['external_urls']['spotify'] }}" class="track-item" id="spantop">{{ $album['name'] }}</a></div>
                                <div id="bottext"><span class="track-item" id="spanbot">{{ $album['artists'][0]['name'] }}</span></div>
                            </div>
                    @endforeach
                @endif
                @if(!$playlists == '')
                    <div id="Playlist" class="favres"><span class="text-item">Le tue Playlist</span></div>
                    @foreach ($playlists as $playlist) 
                            <div id="elembox" name = "playlist">
                                <div id="buttonbox">
                                    <img name="remove" class="button" id="favbutton" src="http:/example-app/public/img/remove.png">
                                </div>
                                <div id="box_cover">
                                    <a id="elemid" value="{{ $playlist['id'] }}"></a>
                                    <img id="cover" src="{{ $playlist['images'][0]['url'] }}">
                                </div>
                                <div id="toptext"><a href = "{{ $playlist['external_urls']['spotify'] }}" class="track-item" id="spantop">{{ $playlist['name'] }}</a></div>
                                <div id="bottext"><span class="track-item" id="spanbot">{{ $playlist['owner']['display_name'] }}</span></div>
                            </div>
                    @endforeach
                @endif
                </div>
