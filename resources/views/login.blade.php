<html>
    ...
    <body>
        <div id="box">
            ...
            <div id='splashpage'> 
                <h1 class="text-item">Benvenuto in Spotihelper!</h1>
                <p class="text-item">Inserisci le tue credenziali per accedere</p>
                @if(isset($login_error))
                    <span class='error' id='login_error'>Username e/o Password non validi!</span>
                @endif
            </div>
            <div id="reg">
            <form method='post' autocomplete='off'>
                <input type='hidden' name='_mode' id='l_mode' value='Login'>
                <input type='hidden' name='_token' value='{{ $csrf_token }}'>
                <label for="l_usr" class="text-item">Username</label> 
                <input type="text" name='l_user' onfocus="select()" id="l_usr" class='flex-item' value="Username"/>
                    
                <label for="l_pwd" class="text-item">Password</label>
                <input type="Password" name='l_pass' onfocus="select()" id="l_pwd" class='flex-item' value="Password"/>
                
                <input type='submit' value='Login'>

                <div id="ax">
                    <a class="text-item">Non hai un account?</a>
                    <a id="signuplink" class="text-item" href='https://'>Registrati.</a>
                </div>
            </form>
            </div>
            <div id="bgi">
                <img src="{{ url('img/bg.jpg') }}"></img>
            </div>
            </div>
            ...
            <div id="splashpage"> 
                <h1 class="text-item">Compila tutti i campi per registrarti</h1>
                <p class="text-item">Crea un nuovo account per utilizzare gli strumenti di Spotihelper</p>
                <span class='hidden' id='user_error'>Username già in uso!</span>
                <span class='hidden' id='email_error'>E-mail già in uso!</span>
            </div>
            <div id="reg">
                <form method='post' autocomplete='off'>
                    <input type='hidden' name='_mode' id='s_mode' value='Signup'>
                    <input type='hidden' name='_token' value='{{ $csrf_token }}'>
                    <label for="s_usr" class="text-item">Username</label>
                    <input type="text" name="s_user" onfocus="select()" id="s_usr" class='flex-item' value="Username"/>

                    <label for="s_pwd" class="text-item">Password (Min. 8 caratteri)</label>
                    <input type="Password" name="s_pass" onfocus="select()" id="s_pwd" class='flex-item' value="Password"/>

                    <label for="s_cpwd" class="text-item">Conferma Password</label>
                    <input type="Password" name="s_cpass" onfocus="select()" id="s_cpwd" class='flex-item' value="Password"/>

                    <label for="s_mail" class="text-item">E-mail</label>
                    <input type="text" name="s_email" onfocus="select()" id="s_mail" class='flex-item' value="Email"/>

                    <input type='submit' id='s_submit' value='Signup' disabled>
                    <div id="ax">
                        <a id="loginlink" class="text-item" href='https://'>Ho gia un account.</a>
                    </div>
                </form>
            </div>
            <div id="bgi">
                <img src="https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322792/dancing-in-the-club.jpg"></img>
            </div>
        </div>
    </div>
        <footer>
            <span class="text-item">Giuseppe Masaracchio N.M. O46001395</span>
        </footer>
    </body>
</html>